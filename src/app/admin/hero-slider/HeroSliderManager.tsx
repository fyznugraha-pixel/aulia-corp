...
export function HeroSliderManager({ initialSlides }: { initialSlides: HeroSlider[] }) {
  ...
  const [croppedMobileImage, setCroppedMobileImage] = useState<File | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (croppedMobileImage) {
      formData.set('mobileImageFile', croppedMobileImage);
    }
    
    const file = formData.get('imageFile') as File;
    const mfile = formData.get('mobileImageFile') as File;
    if ((file && file.size > 0) || (mfile && mfile.size > 0)) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="p-5 md:p-6 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50">
        ...
        <button 
          onClick={() => { setEditingSlide(null); setCroppedImage(null); setCroppedMobileImage(null); setIsSlideOverOpen(true); }}
            ...
            {initialSlides.map(s => (
              <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 text-right">
                  <button onClick={() => { setEditingSlide(s); setCroppedImage(null); setCroppedMobileImage(null); setIsSlideOverOpen(true); }} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg mr-2 transition-colors">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-slate-50 p-5 rounded-xl border border-slate-100 space-y-4">
              ...
              <div className="mt-4 pt-4 border-t border-slate-200">
                <h4 className="text-sm font-semibold text-slate-700 mb-2">Gambar Versi Mobile (Opsional)</h4>
                <p className="text-xs text-slate-500 mb-3">Jika dikosongkan, gambar utama akan ditampilkan di layar mobile.</p>
                <ImageCropper 
                  label="Upload Gambar Mobile (9:16)"
                  aspect={9/16} // Mobile vertical aspect
                  defaultPreview={editingSlide?.mobileImageUrl || undefined}
                  onCropComplete={setCroppedMobileImage}
                  removable={true}
                />
                {editingSlide?.mobileImageUrl && <input type="hidden" name="mobileImageUrl" value={editingSlide.mobileImageUrl} />}
              </div>
            </div>
